...
void Graphics::drawTrafficObjects()
{
    ...
    for (auto it : _trafficObjects)
    {
        ...
        else if (it->getType() == ObjectType::objectBicycleIntersection)
        {
            cv::RNG rng(it->getID());
            //int b = rng.uniform(255, 0);
            //int g = rng.uniform(0, 255);
            //int r = sqrt(255*255 - g*g - r*r); // ensure that length of color vector is always 255
            cv::Scalar vehicleColor = cv::Scalar(255,0,0);
            cv::circle(_images.at(1), cv::Point2d(posx, posy), 15, vehicleColor, -1);
        }
        else if (it->getType() == ObjectType::objectBicycle)
        {
            cv::RNG rng(it->getID());
            //int b = rng.uniform(255, 0);
            //int g = rng.uniform(0, 255);
            //int r = sqrt(255*255 - g*g - r*r); // ensure that length of color vector is always 255
            cv::Scalar vehicleColor = cv::Scalar(0,255,255);
            cv::circle(_images.at(1), cv::Point2d(posx, posy), 15, vehicleColor, -1);
        }
    }
